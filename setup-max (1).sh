...
setup_acme_ssl() {
    ...
    # ── 2. Hentikan Nginx sementara (standalone mode port 80) ─
    local nginx_was_up=0
    if is_up nginx; then
        nginx_was_up=1
        systemctl stop nginx 2>/dev/null
    fi

    # ── 3. Issue SSL untuk domain ──────────────────────────────
    inf "Issue SSL untuk domain: ${W}${domain}${NC} (standalone mode port 80)..."
    "$ACME_BIN" --issue --standalone -d "$domain" \
        --httpport 80 --force 2>&1 | grep -E "(success|error|Cert|Domain|skip)" | \
        while read -r ln; do inf "  $ln"; done
    ...
    # Nyalakan Nginx kembali
    [[ "$nginx_was_up" == "1" ]] && systemctl start nginx 2>/dev/null

    if [[ "$issue_ok" == "0" ]]; then
        warn "acme.sh standalone gagal — coba webroot (Nginx harus jalan)..."
        # Fallback: webroot via Nginx
        mkdir -p /var/www/html/.well-known/acme-challenge
        systemctl start nginx 2>/dev/null
        sleep 2
        "$ACME_BIN" --issue -d "$domain" -w /var/www/html --force 2>&1 | \
            grep -E "(success|error|Cert|Domain|skip)" | \
            while read -r ln; do inf "  $ln"; done
        if [[ -f "$ACME_DIR/${domain}/${domain}.cer" ]]; then
            issue_ok=1
        fi
    ...
    if [[ "$issue_ok" == "0" ]]; then
        ...
        err "Pastikan:"
        err "  1. Domain ${domain} sudah pointing ke IP server ini"
        err "  2. Port 80 tidak diblokir firewall"
        warn "Fallback ke SSL self-signed — fitur NLTS/SSL tidak optimal"
        # Buat self-signed sebagai fallback agar panel tetap jalan
        gen_selfsigned_ssl
        return 1
    fi

    # ── 4. Install/copy cert ke direktori yang dipakai panel ──
