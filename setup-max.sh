...
SS_DB="$DIR/ss-users.db";          SLOW_DB="$DIR/slowdns-users.db"
...
SLOW_DIR="/etc/slowdns";            SLOW_BIN="/usr/local/bin/sldns-server"
...
OHP_URL="${BIN_REPO}/ohpserver";    ACME_URL="${BIN_REPO}/acme.sh"
...
draw_vps() {
    ...
    # Baris 1: SSH | Dropbear | Stunnel | Nginx | WS-SSH  → info port Nginx & WS
    _btn "  ${DIM}SSH${NC}${ssh_b} ${DIM}DR${NC}${dr_b} ${DIM}STN${NC}${stun_b} ${DIM}NGX${NC}${ngx_b} ${DIM}WS${NC}${ws_b}  ${A1}│${NC}  ${DIM}Nginx:${NC}${Y}80/443/8443${NC}  ${DIM}WS-SSH:${NC}${Y}/ws-ssh${NC}"
    # Baris 2: Xray | Trojan-Go | Hysteria | OpenVPN | WireGuard → info protokol
    _btn "  ${DIM}XRY${NC}${xray_b} ${DIM}TGO${NC}${tgo_b} ${DIM}HY${NC}${hy_b} ${DIM}OVPN${NC}${ovpn_b} ${DIM}WG${NC}${wg_b}  ${A1}│${NC}  ${DIM}Xray:${NC}${Y}vmess/vless/trojan${NC}"
...
_show_ssh_payload() {
    ...
    echo -e "  ${A1}└──────────────────────────────────────────${NC}"
    echo ""
    echo -e "  ${Y}▸ Payload 4 — SlowDNS (DNS Tunnel)${NC}"
    echo -e "  ${A1}┌──────────────────────────────────────────${NC}"
    echo -e "  ${A1}│${NC} ${BLD}Server:${NC} ${W}${dom}${NC}  ${BLD}DNS Port:${NC} ${Y}53${NC}  ${BLD}Server Port:${NC} ${Y}5300${NC}"
    echo -e "  ${A1}│${NC} ${BLD}Username :${NC} ${W}${u}${NC}  ${BLD}Password:${NC} ${W}${p}${NC}  ${BLD}Mode:${NC} ${Y}DNS/UDP${NC}"
    echo -e "  ${A1}└──────────────────────────────────────────${NC}"
    echo ""
    echo -e "  ${Y}▸ Payload 5 — HTTP Proxy CONNECT${NC}"
    echo -e "  ${A1}┌──────────────────────────────────────────${NC}"
    echo -e "  ${A1}│${NC} ${BLD}Server:${NC} ${W}${dom}${NC}  ${BLD}Port:${NC} ${Y}80${NC}"
    echo -e "  ${A1}│${NC} ${BLD}Payload  :${NC}"
...
install_all_bins() {
    ...
    [[ ! -x "$OHP_BIN" ]] && _dl_bin "$OHP_URL" "$OHP_BIN" 100000 "OHP Server" || ok "  ✓ OHP ada — skip"
    ...
    if [[ ! -x "$UDPGW_BIN" ]]; then
        ...
        if dl "$UDPGW_URL" "$tmp/badvpn-master.zip"; then
            ...
            local ub; ub=$(find "$tmp/badvpn" -type f \( -name "badvpn-udpgw" -o -name "udpgw" \) 2>/dev/null | head -1)
            if [[ -n "$ub" ]] && verify_binary "$ub" 10000; then
                install -m755 "$ub" "$UDPGW_BIN"; ok "    ✓ BadVPN UDPGW"; ok_count=$((ok_count+1))
            else warn "    ✗ BadVPN tidak ditemukan"; fail_count=$((fail_count+1)); fi
        else warn "    ✗ Gagal download BadVPN"; fail_count=$((fail_count+1)); fi
    else ok "  ✓ BadVPN ada — skip"; fi
...
install_udpgw() {
    ...
    if [[ ! -x "$UDPGW_BIN" ]]; then
        ...
        if dl "$UDPGW_URL" "$tmp/badvpn-master.zip"; then
            ...
            local ub; ub=$(find "$tmp" -type f \( -name "badvpn-udpgw" -o -name "udpgw" \) 2>/dev/null | head -1)
            if [[ -n "$ub" ]] && verify_binary "$ub" 10000; then
                install -m755 "$ub" "$UDPGW_BIN"; ok "BadVPN UDPGW terpasang"
            else err "Binary udpgw tidak ditemukan"; rm -rf "$tmp"; return 1; fi
        else err "Gagal download BadVPN"; rm -rf "$tmp"; return 1; fi
        rm -rf "$tmp"
    else ok "udpgw ada — skip"; fi
...
# ════════════════════════════════════════════════════════════
install_slowdns() {
    inf "Install SlowDNS..."; mkdir -p "$SLOW_DIR"
    if [[ ! -x "$SLOW_BIN" ]]; then
        if dl "$SLOWDNS_URL" "$SLOW_BIN"; then
            chmod +x "$SLOW_BIN"
            verify_binary "$SLOW_BIN" 100000 || { warn "Binary SlowDNS terlalu kecil"; rm -f "$SLOW_BIN"; }
        else warn "Gagal download SlowDNS"; fi
    else ok "SlowDNS ada — skip"; fi
    [[ -x "$SLOW_BIN" && ! -s "$SLOW_DIR/server.priv" ]] && \
        "$SLOW_BIN" -gen-key -privkey-file "$SLOW_DIR/server.priv" -pubkey-file "$SLOW_DIR/server.pub" &>/dev/null || true
    if [[ -x "$SLOW_BIN" ]]; then
...
WantedBy=multi-user.target
SLDEOF
        systemctl daemon-reload; systemctl enable slowdns &>/dev/null
        iptables -t nat -C PREROUTING -p udp --dport 53 -j REDIRECT --to-ports 5300 2>/dev/null \
            || iptables -t nat -A PREROUTING -p udp --dport 53 -j REDIRECT --to-ports 5300
        netfilter-persistent save &>/dev/null; systemctl restart slowdns 2>/dev/null
        ok "SlowDNS aktif (53 → 5300)"
    fi
}
...
# ════════════════════════════════════════════════════════════
#  INSTALLER — OHP (opsional)
# ════════════════════════════════════════════════════════════
install_ohp() {
    inf "Install OHP (opsional)..."
    if [[ ! -x "$OHP_BIN" ]]; then
        if dl "$OHP_URL" "$OHP_BIN" && chmod +x "$OHP_BIN" && verify_binary "$OHP_BIN" 100000; then
            ok "OHP terpasang"
        else rm -f "$OHP_BIN" 2>/dev/null; warn "OHP gagal — dilewati"; return; fi
    fi
...
WantedBy=multi-user.target
OHPEOF
    systemctl daemon-reload; systemctl enable ohp &>/dev/null; systemctl restart ohp 2>/dev/null
...
do_install_all() {
    ...
    _step  2 "Update apt + dependencies";           install_deps
    _step  3 "Setup direktori";                     mkdir -p "$DIR" "$LOGDIR" "$BACKUPDIR"; ok "Siap"
    _step  4 "Download binary chanelog/bin";        install_all_bins
    _step  5 "SSL acme.sh → ${inp_domain}";        setup_acme_ssl "$inp_domain" || gen_selfsigned_ssl
    ...
    _step  9 "Hysteria 2 (UDP 36712)";             install_hysteria
    ...
    _step 13 "SlowDNS (53 → 5300)";               install_slowdns
    _step 14 "WebSocket SSH Proxy";               install_ws_ssh_proxy
    _step 15 "Nginx reverse-proxy";               install_nginx
    _step 16 "Cron jobs + OHP + BBR";             install_cron_jobs; install_ohp; enable_bbr_silent
...
slow_add() {
    show_header; _top; _btn "  ${IT}${AL}➕  TAMBAH SLOWDNS${NC}"; _bot; echo ""
    echo -ne "  ${A3}Username${NC}: "; read -r u; [[ -z "$u" ]] && { pause; return; }
    ...
    echo -ne "  ${A3}Masa aktif (hari)${NC} [30]: "; read -r d; [[ ! "$d" =~ ^[0-9]+$ ]] && d=30
    echo -ne "  ${A3}Max Login${NC} [2]        : "; read -r ml; [[ ! "$ml" =~ ^[0-9]+$ ]] && ml=2
    local exp; exp=$(mk_exp "$d"); useradd -e "$exp" -s /bin/false -M "$u" 2>/dev/null
    echo -e "${p}\n${p}" | passwd "$u" &>/dev/null; echo "${u}|${p}|${exp}|${ml}" >> "$SLOW_DB"; set_maxlogin "$u" "$ml"
    local dom pub; dom=$(get_domain); pub=$(cat "$SLOW_DIR/server.pub" 2>/dev/null || echo "-")
    printf "\n  ${LG}✅ SlowDNS${NC}\n  User: ${W}%s${NC}  Pass: ${A3}%s${NC}  NS: ${W}%s${NC}  Port: ${Y}53/5300${NC}\n  PubKey: ${DIM}%s${NC}\n\n" "$u" "$p" "$dom" "$pub"
    pause
}
slow_del()   { show_header; _top; _btn "  ${IT}${AL}🗑   HAPUS SLOWDNS${NC}"; _bot; echo ""; xray_list_compact "$SLOW_DB" "SlowDNS"; echo ""; echo -ne "  ${A3}Username${NC}: "; read -r u; grep -q "^${u}|" "$SLOW_DB" || { err "Tidak ada!"; pause; return; }; userdel -r "$u" 2>/dev/null; sed -i "/^${u}|/d" "$SLOW_DB"; del_maxlogin "$u"; ok "SlowDNS ${W}${u}${NC} dihapus"; pause; }
slow_renew() { show_header; _top; _btn "  ${IT}${AL}🔁  PERPANJANG SLOWDNS${NC}"; _bot; echo ""; xray_list_compact "$SLOW_DB" "SlowDNS"; echo ""; echo -ne "  ${A3}Username${NC}: "; read -r u; grep -q "^${u}|" "$SLOW_DB" || { err "Tidak ada!"; pause; return; }; echo -ne "  ${A3}Tambah hari${NC} [30]: "; read -r d; [[ ! "$d" =~ ^[0-9]+$ ]] && d=30; local cur new; cur=$(grep "^${u}|" "$SLOW_DB" | cut -d'|' -f3); new=$(TZ="Asia/Jakarta" date -d "${cur} +${d} days" +"%Y-%m-%d" 2>/dev/null || mk_exp "$d"); sed -i "s|^\(${u}|[^|]*|\)[^|]*|\1${new}|" "$SLOW_DB"; chage -E "$new" "$u" 2>/dev/null; ok "→ ${Y}${new}${NC}"; pause; }
slow_list()  { show_header; _top; _btn "  ${IT}${AL}📋  LIST SLOWDNS${NC}"; _bot; echo ""; xray_list_pretty "$SLOW_DB" "SlowDNS" "Password"; pause; }
...
tool_restart_all() {
    ...
    local svcs=(ssh sshd dropbear stunnel4 xray trojan-go hysteria-server \
                ...
                ws-ssh-proxy badvpn-udpgw-7100 badvpn-udpgw-7200 badvpn-udpgw-7300 slowdns ohp)
    for s in "${svcs[@]}"; do
...
tool_check_service() {
    ...
    local svcs=(ssh dropbear stunnel4 nginx ws-ssh-proxy xray trojan-go \
                ...
                wg-quick@wg0 badvpn-udpgw-7100 badvpn-udpgw-7200 badvpn-udpgw-7300 \
                slowdns ohp cron)
...
do_backup() {
    ...
    local files=(); for f in "$DIR" /etc/xray /etc/trojan-go /etc/hysteria /etc/wireguard /etc/openvpn /etc/stunnel /etc/slowdns /etc/ssh/sshd_config /etc/nginx/conf.d; do [[ -e "$f" ]] && files+=("$f"); done
...
menu_slowdns()  { while true; do show_header; _top; _btn "  ${IT}${AL}🌐  SLOWDNS — 53 → 5300${NC}"; _sep; _btn "  ${A2}[1]${NC} ➕ Buat  ${A2}[2]${NC} 🗑  Hapus  ${A2}[3]${NC} 🔁 Perpanjang  ${A2}[4]${NC} 📋 List  ${A2}[5]${NC} 🔑 PubKey  ${LR}[0]${NC} ◀"; _bot; echo ""; echo -ne "  ${A1}›${NC} "; read -r ch; case $ch in 1) slow_add;; 2) slow_del;; 3) slow_renew;; 4) slow_list;; 5) cat "$SLOW_DIR/server.pub" 2>/dev/null || warn "PubKey tidak ada"; pause;; 0) break;; *) warn "Tidak valid"; sleep 1;; esac; done; }
...
menu_about() {
  ...
  BadVPN UDPGW: 7100,7200,7300 | OHP: 8080
...
do_uninstall() {
    ...
    for s in xray trojan-go hysteria-server wg-quick@wg0 stunnel4 dropbear \
             openvpn-server@tcp openvpn-server@udp slowdns ohp ws-ssh-proxy \
             badvpn-udpgw-7100 badvpn-udpgw-7200 badvpn-udpgw-7300 nginx; do
        systemctl stop "$s" 2>/dev/null; systemctl disable "$s" 2>/dev/null
    done
    rm -rf /etc/xray /etc/trojan-go /etc/hysteria /etc/slowdns "$WS_DIR"
    rm -f "$XRAY_BIN" "$TROJANGO_BIN" "$HY_BIN" "$UDPGW_BIN" "$OHP_BIN" "$SLOW_BIN"
    rm -f /etc/systemd/system/xray.service /etc/systemd/system/trojan-go.service \
          /etc/systemd/system/hysteria-server.service /etc/systemd/system/slowdns.service \
          /etc/systemd/system/ws-ssh-proxy.service /etc/systemd/system/ohp.service \
          /etc/systemd/system/badvpn-udpgw-*.service /etc/cron.d/maxpanel-*
...
main_menu() {
    while true; do
        ...
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${A2}" "${A2}" "[7]  🌍  SlowDNS               " "[8]  ⚙   System Tools"
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${A2}" "${A2}" "[9]  💾  Backup & Restore      " "[10] 🎨  Tema"
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${A2}" "${A2}" "[11] ⚙️  Pengaturan            " "[12] 🔄  Update Script"
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${A2}" "${A4}" "[13] ℹ️  About                 " "[14] 🚀  Install Ulang"
        echo -e "  ${A1}${_DASH}${NC}"
        _r2 "${LR}" "${LR}" "[E]  🗑  Uninstall             " "[X]  ✗   Keluar"
        echo -e "  ${A1}${_DASH}${NC}"
        ...
        case ${ch,,} in
            ...
            7)  menu_slowdns ;;   8)  menu_system ;;      9)  menu_backup ;;
            10) menu_tema ;;      11) menu_settings ;;   12) cek_update ;;
            13) menu_about ;;     14) do_install_all ;;
            e)  do_uninstall ;;
    ...
    case "${1:-}" in
        ...
        --auto-backup)
            ...
            tar -czPf "$out" "$DIR" /etc/xray /etc/trojan-go /etc/hysteria \
                /etc/wireguard /etc/openvpn /etc/stunnel /etc/slowdns 2>/dev/null
