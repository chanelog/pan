...
do_install_all() {
    ...
    echo -e "\n  ${A1}${_DASH}${NC}"; inf "Mulai instalasi → 18 langkah..."; echo -e "  ${A1}${_DASH}${NC}\n"

    _step() { CURRENT_STEP="$2"; echo -e "\n  ${A4}[${1}/18]${NC} ${BLD}${AL}${2}${NC}"; echo -e "  ${A1}${_DASH}${NC}"; }
    ...
    _step 12 "BadVPN UDPGW (7100/7200/7300)";      install_udpgw
    _step 13 "OpenVPN (TCP 1194 + UDP 2200)";      install_openvpn
    _step 14 "WireGuard (UDP 51820)";              install_wireguard
    _step 15 "SlowDNS (port 53 → 5300)";           install_slowdns

    # ── Tahap 7: Reload semua service pakai cert final ───────────────
    _step 16 "Reload semua service dengan cert terbaru"
    ...
    # ── Tahap 8: Finalisasi ──────────────────────────────────────────
    _step 17 "Cron jobs + OHP server";              install_cron_jobs; install_ohp
    _step 18 "BBR + kernel tuning"
    enable_bbr_silent
